...
public final class TurnResolver {
    ...
    public static TurnResult resolve(GameState input) {
        ...
        for (int register = 1; register <= Robot.REGISTER_COUNT; register++) {
            ...
            checkpoints.touch();
            endGameIfOneRobotIsLeft(state, log);
    ...
    /**
     * Ends the game if every robot but one has been eliminated (design.md 2.10), or if every
     * robot has been (a game without a winner). Does nothing in a game with fewer than two
     * robots, or if the game is already over.
     *
     * @param state the game state
     * @param log   receives the {@link GameEvent.GameEnded} event
     */
    private static void endGameIfOneRobotIsLeft(GameState state, EventLog log) {
