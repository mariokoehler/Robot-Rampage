...
Connect, Lobby, static board renderer, programming screen, turn replay) done** — 343 unit tests in `core` plus 11 integration tests in `server` (real sockets, threads). A whole turn can be resolved headlessly:
...
sprites in `assets/board` are generated by `tools/design-import/make-board-sprites.js` (run it after changing the SVGs).
...
`completeResolution()`; never apply them earlier. The server's turn pause (`SessionConfig.defaults()`) is the playback budget.
