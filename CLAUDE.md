...
**Autosave scratched, not deferred** (owner, 2026-09-26 — design.md 3.10: no persistence, no accounts, this isn't
that kind of game). **Board renderer (slice 3):** `client.render.BoardActor` + libGDX-free `client.board` (`BoardGeometry`, `RobotPose`);
...
`StartupScreen`'s constructor, which re-runs on every "Back" from Connect). **Verified by the owner's own playtest,
not `ScreenSnapshot`/`GameScreenDriver`** (2026-09-26): the Settings dialog and the Lobby stepper look fine in-game.
They were never pixel-checked via those tools — `ScreenSnapshot` lives in a different package than the
package-private `SettingsDialog` and only drives `GameScreen`, so wiring it in would have been real plumbing, not a
quick addition (design decision, not an oversight — see design.md 4.6) — and the owner's playtest was accepted as
the verification path instead, per this file's testing conventions.
...
side, right click removes it) and throws if the mapping is wrong. **Drag feel, keyboard shortcuts and the close-window
prompt were never covered by `EditorSnapshot`** (no pixel/automated check for those three), but the owner's in-app
check confirmed they're all fine (2026-09-26) — no outstanding concern here. **Never create a Scene2D `Table` (or anything that makes a `Cell`) in a field initializer of the
...
(`BoardMetricsTest.provingGrounds()`). `EditorSnapshot` now waits for all 20 bot games and writes `editor-metrics.png` (the README screenshot).
...
**Design backlog closed out (owner, 2026-09-26):** game length/pacing and any 2016-edition ideas are settled as "keep
as-is" (design.md 7 `DECISION:` notes); autosave/persistence/accounts are scratched outright, not deferred (design.md
...
- **README screenshots** (`docs/screenshots/programming.png`, `resolution.png`, `game-over.png`) are `ScreenSnapshot`'s
  ...
  `System.Drawing` in PowerShell, HighQualityBicubic). `board-editor.png` and `board-editor-suggestions.png` are
  `EditorSnapshot`'s `editor-metrics.png` and `editor-suggestions.png`, scaled the same way. They are not regenerated by
  any build — redo them after a visible UI change, or they silently drift from the real game.
